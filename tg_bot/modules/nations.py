...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUPPORT_USERS:
        rt += "Requested Legend Union to promote a Support user to Sudo."
        SUPPORT_USERS.remove(user_id)

    if user_id in WHITELIST_USERS:
        rt += "Requested Legend Union to promote a Whitelist user to Sudo."
...
def addsupport(
    ...
    if user_id in SUDO_USERS:
        rt += "Requested Legend Union to demote this Sudo to Support"
    ...
    if user_id in WHITELIST_USERS:
        rt += "Requested Legend Union to promote this Whitelist user to Support"
        WHITELIST_USERS.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("Requested Legend Union to demote this user to Civilian")
        SUDO_USERS.remove(user_id)
...
\n*Legend Union* - Devs who can access the bots server and can execute, edit, modify bot code. Can also manage other Nations
...
Report abuse or ask us more on these at [Legend Union](https://t.me/opamanxworld).
"""
...
@kigcmd(command=["devlist", "legend"])
@whitelist_plus
@rate_limit(40, 60)
def devlist(update: Update, context: CallbackContext):
    bot = context.bot
    true_dev = list(set(DEV_USERS) - {OWNER_ID})
    reply = "<b>Legend Union Members :</b>\n"
